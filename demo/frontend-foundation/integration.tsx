...
function Harness(): React.JSX.Element {
  ...
  return <ConfigProvider theme={{ ...readAntdSemanticTheme(), algorithm: dark ? theme.darkAlgorithm : theme.defaultAlgorithm }}><AntdApp><div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}><header style={{ padding: 12, display: 'flex', gap: 16 }}><strong>正式组件集成验收 / 仅内存</strong><button id="toggle-theme" onClick={() => { document.documentElement.classList.toggle('theme-dark', !dark); setDark(!dark) }}>切主题</button><button id="reset-sample" onClick={reset}>重置样本</button></header><Host /></div></AntdApp></ConfigProvider>
}
