...
describe('Muya note editor demo', () => {
  it('启动真实 Muya 内核且不存在模拟渲染路径', async () => {
    ...
    const runtime = await readFile('src/renderer/src/components/muya-note/muya-runtime.ts', 'utf8')
    ...
    expect(main).not.toContain("from '@muyajs/core'")
    expect(main).toContain('loadMuyaRuntime()')
