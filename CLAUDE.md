...
| 当前版本 | v0.15.1 公测版 Beta 5 修订（版本史见 docs/changelog/CHANGELOG.md；1.0.0 为正式版） |
...
| BR-008 | 远期：「导出为」——计划导出为 图片/PDF/.plan | .plan 已有（BR-004）；新增图片/PDF 渲染导出（Electron printToPDF 可期；图片可走离屏渲染+capturePage）。用户 2026-09-10 提出，规划保留项 |
...
- 菜单/标题栏必须加 `no-drag` 白名单（历史 bug：事件被窗口拖拽吞掉致点击无效）；自绘窗口控制（`.win-controls`）已在组件级全局 no-drag（2026-09-12 引导页漏配实证后根治），新增**其它**交互控件仍须进容器白名单
...
- `docs/HANDOFF-CURRENT.md` 是当前唯一协作状态源；历史 `HANDOFF-*.md` 仅作追溯，不能替代当前状态。
- 接手、实施前、提交后及暂停前都必须读取并原子更新该文件；先核对 `git status --short --branch`，状态不一致时先交接，禁止直接改代码。
