...
状态：实施中；执行者：Codex；开始：2026-09-24。稳定性修复完成后按用户已授权的发布流程制作 v0.15.1 patch 修订版。
...
- [ ] 推送 main 与 v0.15.1 tag；GitHub Release 上传安装包，Gitee Release 只更新说明/提供 GitHub 下载链接；两端 GET 核验。
- [ ] 重启使用新配置的开发服务器，在正式 NoteCard 首次打开注释，确认页面不 reload 且可编辑；当前运行的 5173 server 未被我重启。
- [ ] 提交本轮变更并推送状态记录。人工检查 IME、真实系统剪贴板、语言选择浮层、连续删除、切卡 flush、亮暗主题及窄窗口，需用户在本机配合完成后才能关闭 Muya 人工验收。
