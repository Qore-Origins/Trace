...
- 发布版本已由用户确认采用 `v0.17.0`（相对 v0.15.1 按任务三态反馈、离线 PlantUML 两个功能域递增）。版本文件、CHANGELOG、累计改动文档和发行说明已更新；全量门槛和 Windows x64 安装包、本地资源/运行时核验已完成，当前只剩提交合并、SSH 双端推送及 GitHub/Gitee Release 创建和核验。
...
- Step7 首轮实现与质量审查（2026-09-26）：提交 `05017c826b4eb456f160c9d8bfb71bc7868973d6`，只改 `src/renderer/src/App.tsx`、两份 zh/en locale 与 `test/plantuml-preferences.spec.ts`；`pref-store.ts` 的 versioned migration、hydration 标记、旧非空 URL→custom 已在 Step1 完成，本步复用未改。该提交定向偏好+无障碍 18/18、全量 35 files / 342 tests、typecheck、build（renderer 7,154 modules）通过。首轮质量复审当时发现的 P2（旧 retry 回包可覆盖新模式状态）已由下一条 `6c72de0` 修复并通过双审；本条保留为历史状态快照。无 Electron UI 人工验收。
- Step7 P2 修复提交 `6c72de0c558e9f9c107eaf14fa54650af83df236`（只含 `src/renderer/src/App.tsx`、`test/plantuml-preferences.spec.ts`），原实现提交仍为 `05017c8`。TDD deferred retry 回归先证实旧 `running`、`starting`、失败回包均能覆盖新模式 stopped，再以 hydration/mode/port 与服务状态代次保护阻断；定向偏好+无障碍 21/21。协调者独立实跑 `npm run typecheck`、`npm run test`（35 files / 345 tests）、`npm run build`（renderer 7,154 modules）、`git show --check HEAD` 均通过。规格审查 Ready: Yes、质量/安全审查 Ready: Yes，无阻断或已证实安全问题。实现边界已释放；Electron 设置 UI/主题/键盘人工验收仍待。

- Step6 审查更新：双阶段审查已一致确认根目录初始化失败会被误报为可用 workspace（Important/P1）；当前代码 HEAD 为 51eabbddc3e1264a2d5d2e770396521b78de03c9。步骤 6 的原七文件边界重新开放给 /root/plantuml_ipc_impl 修复；其余代理只读审查，不与实现重叠。要求 bootstrap 进入可恢复 onboarding，并保证激活失败后重新选库不会被旧目录的确认规则卡住；修复后继续双阶段复审。以下旧 HEAD 记录尚未替换，以上述 HEAD 与本条为准。
...
- 当前工作区：隔离 worktree `D:\Code\Project\Qore\Trace\.worktrees\codex\trace-task10-11`，分支 `codex/trace-task10-11`；代码 HEAD `b31631d43a721db96228ecb64c0da60095137f21`，`main` 为其祖先（本地 `main...HEAD` 实测 0/31）。用户已授权本轮完成发布，当前准备先复核并提交登记的版本/发布文档，再将分支快进合入 `main` 并 SSH 推送。主目录未跟踪的 `Resource/pic/`、`Resource/vid/` 是用户资源，必须保留且不触碰。
...
- 当前发布阶段文件所有权由 Codex 持有：`AGENTS.md`、`CLAUDE.md`、`builds/build_history.json`、`builds/release_notes/release_notes_v0.17.0.md`、`docs/HANDOFF-CURRENT.md`、`docs/Plan/README.md`、v0.17.0 对应计划 MD/JSON、CHANGELOG、累计改动文档、lifecycle 发布说明、`package.json` 与 `package-lock.json`、已批准规格中的版本同步；实施及复审代码边界已释放。
- 精确文件所有权已登记：
  - Step8 补充登记：`/root/plantuml_step8_renderer_impl` 另拥有 `vitest.config.ts`，范围包括启用与 Electron renderer 一致的 Vitest client dependency optimizer，以及 happy-dom 下挂载真实 React/Muya 编辑器所需测试夹具；测试复现 optimized `@muyajs/core` 与 vendor-source loader 的双缓存，不 mock runtime/loader/preloader。用户于 2026-09-26 明确批准：仅在测试构建中对 Muya Prism 语言注册里精确的 `import.meta.glob` 作等价展开，以解决 Vite 专属 glob 在 happy-dom 优化包中的运行限制；其余 JS/TS、runtime、PlantUML loader/chunk 和网络请求均不得拦截或替换。不得改 `vendor/muya` 与指纹，生产构建配置和行为不受测试转换影响。
  - Step8 测试门控补充已登记：`/root/plantuml_step8_renderer_impl` 可在 `vitest.config.ts` 的 client esbuild optimizer 增加仅测试插件，对精确的 PlantUML renderer 模块注入延迟 gate，用来构造可复现的真实异步竞态；不得写入 `vendor/muya`、不得 mock runtime/loader/preloader，生产构建配置不受影响。已用 client esbuild `onResolve` 将测试中的精确 `plantuml-encoder` import 指向 `browser-index.js`，与 Electron Vite 既有优化产物一致；优化产物已确认 gate 位于真实 PlantUML renderer 模块前。Node/happy-dom 不能直接加载 client optimizer 输出里的绝对路径静态资源，测试插件仅 stub CSS/图片/字体导入。现允许且只允许另对上述 Prism `import.meta.glob` 作测试专用等价转换；禁止拦截其他 JS/TS、runtime/loader/chunk 或图像请求。真实生命周期 RED 与修复状态见下方最新阶段记录。
  - Step8 有效 RED（2026-09-26）：按用户批准的 Prism 单点测试转换后，实施代理报告实跑 `npx vitest run test/muya-note-integration.spec.tsx -t "does not issue a stale custom PlantUML request" --reporter=verbose`。happy-dom 真正挂载 React 与优化的 `@muyajs/core`，真实 PlantUML chunk 的 gate 已启动；切换 Off 并释放 gate 后，fetch interceptor 捕获到旧 custom endpoint `https://trace-stale-note.invalid/plantuml/svg/...`，解码请求含哨兵 `TRACE_PRIVATE_PLANTUML_SOURCE`，零泄漏断言按预期失败（1 个实际请求）。这是本轮首个有效目标 RED；生产文件仍未改。当前只授权同一实现者在已登记的 `MuyaNoteEditor.tsx` 与 `muya-config.ts` 尝试同 loader 实例预加载/失效，之后重跑该 RED；随后需独立规格与质量/安全复审。
  - Step8 实现提交（2026-09-26）：`b31631d43a721db96228ecb64c0da60095137f21`（`fix(note): share Muya PlantUML renderer preload`），仅含登记的六个代码/测试文件。预加载 import 改为 `@muyajs/core/utils/diagram/index.js`，并将此精确子路径纳入 Electron `optimizeDeps.include`，Vitest/Electron 优化 metadata 证明与 `@muyajs/core` 共用一个含单一 `rendererCache` 的 chunk。首轮规格/质量审查曾发现 happy-dom 请求拦截越界（Ready: No）；按授权将唯一测试文件改为 3 个真实 loopback HTTP 服务，服务实际接收并记录 PlantUML 图像 GET、返回 SVG，无 fetch/image 请求拦截；旧 custom 与旧本地端口零请求，新端口收到含源码哨兵的请求，代码块保持可编辑。修正后规格审查 Ready: Yes、质量/安全审查 Ready: Yes，均无 P1/P2/P3 阻断；非阻断备注：用例确认 contenteditable 存在，但未额外模拟键盘输入。协调者实跑 focused integration 17/17、`npm run typecheck`、`npm run test`（39 files / 388 tests）、`npm run build`（renderer 7,155 modules）、`git diff --check -- test/muya-note-integration.spec.tsx` 均通过；`happy-dom@20.14.5` 仍为 dev-only。未启动 packaged Electron 窗口；该限制仍须与安装包验证区分。
  - 三态反馈：子代理 `/root/task_state_css_fix` 已在 `85d158f` 完成 `src/renderer/src/styles/cards.css`、`test/frontend-foundation-css.spec.ts`；定向回归先红后绿，规格审查与质量审查通过；代码边界已释放。Electron 亮/暗主题及鼠标/键盘三态循环仍待人工验收。
  - PlantUML 步骤 1 偏好迁移：由 `/root/plantuml_preferences` 完成，提交 `343ff18355eb840bbc485c50e1871350273f53fa` 与修复提交 `c156d69efbc8927f92ffa039ed4c367bd43fe9f3`；范围为 `src/shared/plantuml-types.ts`、`src/renderer/src/stores/pref-store.ts`、`test/plantuml-preferences.spec.ts`，边界已释放。规格复审与质量复审均通过；无未解决 Critical/Important/Minor。最终实施验证：定向测试 8/8、全量 `npm run test` 31 files / 270 tests passed、`npm run typecheck` 通过、`git diff --check` 通过。
  - PlantUML 步骤 2 上游产物/许可证锁定：子代理 `/root/plantuml_artifact_lock` 已提交 `79acec82929643b5020fbb09ba033ce0b7677e6b`，边界为 `scripts/plantuml-runtime.lock.json`、`scripts/plantuml-licenses/PLANTUML-LICENSE.txt`、`scripts/plantuml-licenses/TEMURIN-NOTICE.txt`、`scripts/plantuml-licenses/README.md`；仅此四文件提交。独立规格审查及质量审查均通过，文件边界已释放。实施验证：两项资产字节数/SHA-256 与锁一致、Temurin 官方 sidecar 一致；固定 JDK jdeps 输出与 `modules` 根数组一致；44 个 Temurin legal 文本和两段 PlantUML 许可来源比对通过；本机 PicoWeb/SANDBOX smoke 返回 HTTP 200、2,101-byte SVG，端口已释放。步骤 3 使用最小 jlink runtime 的离线 smoke 通过，确认当前 `modules` 根表足够。清理失败的历史临时验证目录 `C:\Users\aaa\AppData\Local\Temp\trace-plantuml-step2-bc5cc206b79e4b8bb85398285d01f888` 仍保留；执行器之前拒绝删除，待单独精确复核后再决定安全处理，不用替代方式绕过拒绝。
  - PlantUML 步骤 3 精简运行时准备：实现提交 `12d8dd838b20de02e2667821c47215a6053da8e7`，规格问题修复 `66eb291cdcff932615c9da069fae4faa3a87cc16`；两提交仅涉及登记的脚本、测试和步骤3配置文件。新增纯解析回归要求同一 PID/端口的全部 TCP LISTENING 地址严格为 `127.0.0.1`，覆盖另一具体网卡地址拒绝及其他 PID/端口/非监听状态不误判；红/绿定向为 7/7。独立规格复审通过；独立 bug-detector 质量/安全审查 Ready: Yes，无 Critical/Important。Minor 运行约束：`plantuml:prepare` 多进程并发不保证安全，构建流程须串行调用；`.build/plantuml-runtime` 是独占生成目录，不放手工文件。使用锁定 JAR/JDK SHA 构建 jlink runtime，直接路径 `.build/plantuml-runtime/bin/java.exe`；产物 66,542,936 bytes（jlink image 48,584,554 bytes / 145 files），二次 prepare 经 manifest/逐文件 SHA 校验后复用。修复后 smoke HTTP 200 / 有效 2,537-byte SVG，监听 `127.0.0.1:51558`，PID 26984 退出且端口释放，`.smoke` 已清理。验证：`npm run plantuml:prepare` manifest 复用、`npm run typecheck` node/web 通过、全量 `npm run test` 32 files / 277 tests 通过、`git diff --check` 通过（CRLF 提示）。本步骤 cache/staging/smoke 已清理；步骤 2 历史临时验证目录仍待复核，不要误删其他路径。
  - PlantUML 步骤 4 子进程服务：子代理 `/root/plantuml_service_impl` 已提交 `07482c52c05471b8c8782a32b2a7fc567d6b0c41`，仅含 `src/main/services/plantuml-service.ts`、`test/plantuml-service.spec.ts`、`package.json`、`package-lock.json`；代码边界已释放。独立规格/质量审查均 Ready: Yes，无 Critical/Important。Minor 测试覆盖建议：后续集成测试补探针响应体超限、请求取消及原生 `ENOENT` 事件顺序；当前实现已有边界处理，此项不阻塞。`plantuml-encoder@1.4.0` 已作为直接运行依赖，仅提升既有传递依赖，不更新其他包。验证：定向 23/23、全量 33 files / 300 tests、typecheck node/web、测试文件独立 strict tsc、diff-check、`npm ls plantuml-encoder --depth=0` 均通过。stop 等待最多 2 秒；仍活 child 时保留 ownership 并报 `stop_timeout`，禁止替代实例，child 退出后显式 retry。IPC/应用启动接入尚未实施。
  - PlantUML 步骤 5 IPC：实现代理 `/root/plantuml_ipc_impl` 已提交 `665bc0ee8254553005b70ed6712a574a8328fdc6`，仅改 `src/shared/ipc-contract.ts`、`src/shared/event-types.ts`、`src/preload/index.ts`、`src/main/ipc/register.ts`、新建 `test/plantuml-ipc.spec.ts`；规格审查 Ready: Yes、无发现；独立质量/安全审查 Ready: Yes、无已证实 Critical/Important/Minor。质量审查的非阻塞建议是补充访问器属性、Symbol 键和端口下界的拒绝测试，并在 Step6 明确持有 disposer 生命周期。实现边界已释放。验证：RED 16/16；最终 IPC 16/16、服务 23/23、全量 `npm run test` 34 files / 316 tests、`npm run typecheck` node/web、新测试 strict tsc、`git diff --check` 均通过。没有做 Electron 人工验收。
  - PlantUML 步骤 9 离线运行时集成：实现代理 `/root/plantuml_step9_packaging_impl` 已提交 `4875a7a867a8f3c05c0dbc6de7c06015e127fe3a`，仅改 `electron-builder.yml`、`package.json` 的 `build:win` 脚本、新建 `scripts/test-plantuml-runtime.mjs`、新建 `test/plantuml-packaging.spec.ts`、新建 `test/plantuml-runtime-security.spec.ts`。extraResources 把 `.build/plantuml-runtime` 放入 ASAR 外的 `resources/plantuml`，仅排除 owner marker，包含 manifest、Java、JAR、最小运行时和 `LICENSES/` 两份许可；主进程已使用 `process.resourcesPath/plantuml`。TDD RED 2 项；定向 34/34、`npm run typecheck`、脚本语法检查、js-yaml 解析、真实 bundled-runtime smoke（有效 loopback SVG、SANDBOX 本地文件/URL include 无泄漏且 canary 零请求、`-disablestats`、退出释放端口）均通过。构建/安装包资源检查、体积增量和 SHA 尚待协调者串行执行。实现边界释放；不引入依赖，不改计划/HANDOFF/版本文件。
  - Step9 只读质量复审后续：生产打包代码未发现可证明安全漏洞；但运行时安全 smoke 两处断言/清理不足（拒绝请求未验证明确 HTTP 失败语义；`stopChild` 抛错可能跳过 canary/temp cleanup）。现将 `scripts/test-plantuml-runtime.mjs`、`test/plantuml-runtime-security.spec.ts` 精确边界重新登记给 `/root/plantuml_step9_packaging_impl` 补 TDD 回归与安全清理；不得改生产启动参数、打包配置或其他 Step9 文件。未加固前不得把当前 smoke 作为可靠的自动安全门槛。
  - Step9 smoke 门禁提交 `567c9c878ef4b6730e55431dceb2494324629405`（runner + security spec）已完成首轮加固：TDD 新增 5 项先 RED，定向 12/12、`node --check`、真实 bundled-runtime smoke、diff-check 通过；负面验证含 local include 明确 `cannot include`、URL include HTTP 400/SVG/`Cannot open URL`、健康 loopback canary 接收到零 include 请求。独立质量复审确认没有正向 false-pass，Ready: No：`finally` cleanup 抛错可能覆盖原 smoke assertion 错误，CLI 仅打印 `AggregateError.message` 而隐藏 `.errors`；只读 reviewer 用 Node 复现 `originalPreserved:false`。现重新登记给 `/root/plantuml_step9_packaging_impl` 仅改 `scripts/test-plantuml-runtime.mjs`、`test/plantuml-runtime-security.spec.ts`：先加 RED，修复原始 smoke 错误保留、各清理错误可见；保留 runner temp 路径限制。不得改 package/build/生产服务或 Step8 文件。未运行 build/build:win。
  - PlantUML 步骤 6 主窗口启动集成：已完成双阶段审查，完整提交链见上方。实现代码边界已释放；P2 修复仅改 `src/main/services/plantuml-service.ts`、`test/plantuml-service.spec.ts`，已提交 `e13c01339029710be275db1da2d7498c3db123e0`。
  - PlantUML 步骤 7 设置三态与偏好接入：实现 `05017c826b4eb456f160c9d8bfb71bc7868973d6`，P2 修复 `6c72de0c558e9f9c107eaf14fa54650af83df236`；规格与质量/安全双审均 Ready: Yes。父级验证定向 21/21、typecheck、全量 35 files / 345 tests、build（renderer 7,154 modules）、diff-check 通过。实现边界已释放；Electron 设置 UI/主题/键盘人工验收待用户侧完成。
- PlantUML 步骤 8 runtime loader 实例竞态：原实现 `10269efe3a8ba62863a5bce884159665f9781ec6`、首轮 helper 修复 `48dd5ce0443a66d59eda55880f2a2c2912d986ab` 的 TDD/自动验证历史见下文。独立复审 Ready: No，证实 vendor 源码 preload 与 Vite 预优化 `@muyajs/core` 内的实际 renderer loader/cache 不同实例，可能导致切 Off/error/换端口后仍向旧 endpoint 发送注释源码。用户已选择 `happy-dom`（仅开发测试依赖）路径；实施代理 `/root/plantuml_step8_renderer_impl` 重新获得精确边界：`package.json`、`package-lock.json`（仅 happy-dom devDependency）、`electron.vite.config.ts`、`src/renderer/src/components/muya-note/MuyaNoteEditor.tsx`、`src/renderer/src/components/muya-note/muya-config.ts`、`test/muya-note-integration.spec.tsx`。目标是挂载真实 React/Muya 编辑器，证明实际 loader 同实例预加载/失效，覆盖 custom→Off/error/换端口旧 endpoint 不发源码。新需要的文件须先回报协调者更新 HANDOFF 后才扩展所有权；禁止改 `vendor/muya` 与指纹。实现和独立审查未完成，尚无 Step8 修复通过结论。
  - Step9 第二轮 smoke 诊断修复（2026-09-26）：实施代理已提交 `d1fb4a4c99ba5bb5de0c911538e1733124d280c6`，精确范围仅 `scripts/test-plantuml-runtime.mjs`、`test/plantuml-runtime-security.spec.ts`；新增端到端错误链回归，验证主 smoke 错误保留为 `AggregateError.cause`/`.errors[0]`、多个清理错误继续可见且实际 CLI handler 递归输出并返回失败码。实施方报告定向 2 files / 13 tests、`node --check`、`git diff --check`、真实 bundled-runtime smoke 与临时资源清理均通过。独立复审 `/root/plantuml_step9_postfix_review` Ready: Yes，另实跑 security spec 9/9、Node 语法、真实 runtime smoke 和两文件 diff-check。非阻断注意项：人为自引用 AggregateError 可能令 formatter 递归过深（未发现生产可达路径）；原生文件系统错误消息可能含本机临时路径，若未来外传日志需脱敏。代码边界已释放；本项 smoke 门禁补强完成，但不代表 Step9 安装包验收完成。
  - 后续设置/renderer 文件：src/renderer/src/App.tsx、src/renderer/src/i18n/locales/zh-CN.ts、src/renderer/src/i18n/locales/en-US.ts、src/renderer/src/components/cards.tsx、src/renderer/src/components/note-diagram.tsx、src/renderer/src/components/muya-note/MuyaNoteEditor.tsx、src/renderer/src/components/muya-note/muya-config.ts。共享 IPC 合约/preload 文件已单独登记给步骤 5；后续复用 pref-store 前必须先登记新的精确边界。
...
- 阶段区分：设计规格已获用户批准；任务三态修复已通过双阶段审查，但 Electron 亮/暗主题和鼠标/键盘三态人工验收仍待用户侧完成。PlantUML 步骤 1–8 均通过双阶段审查；Step6 shutdown API 针对性复核无缺口；Step9 smoke 诊断补强提交 `d1fb4a4` 已通过独立复审 Ready: Yes。`npm run typecheck`、`npm run test`（39 files / 388 tests）、`npm run build`、`npm run build:win`、包内容与 packaged Java smoke 全部已实跑通过；本地 Release 资料已整理。尚未提交合并/SSH 推送/创建远端 Release。
- 下一步：核对当前所有文档/代码改动与 main 的用户未跟踪资源边界，形成发布提交并快进合入 main；推送 GitHub/Gitee main 与 `v0.17.0` tag；GitHub Release 附安装包，Gitee 仅发说明与 GitHub 下载链接；核验双端 commit/tag、Release 正文和 GitHub 资产摘要。若 Gitee API token 未设置，完成 GitHub 后仅在该 API 步骤暂停并请用户私下设置环境变量。步骤 2 历史临时验证目录仍待精确复核与安全处理。
...
- 发布准备提交 `2316a3b` 与收尾记录提交 `c716b05` 已推送 GitHub/Gitee；两端 `v0.15.0` tag 都指向 `2316a3bf59f4a1a0bd8cd69822a9af57ee4ce017`。
- GitHub Release 安装包为 462,698,904 B，远端 SHA-256 与本地一致；Gitee Release 正文 GET 核验与本地发行说明一致，没有上传安装包（平台自动源码归档除外）。本任务后续另提交稳定性修复，不回改已发布 v0.15.0 tag。
