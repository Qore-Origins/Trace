...
## 当前工作项：注释格式工具栏 + 自动换行（已完成，等用户真机验收）

| 字段 | 状态 |
|---|---|
| 阶段 | **已实现 + 已提交（3fa19a1，双端已推）；等用户真机验收** |
| 产物 | NoteCard 编辑态格式工具栏（粗/斜/删除线/下划线/行内码/代码块/链接 7 钮，选区一键包裹语法）+ 渲染态「自动换行」开关（pre 折行）+ 渲染器扩展（~~删除线~~ / <u>下划线</u>） |
| 已验证 | typecheck 0 错；test 178/178（渲染器 +2：删除线/下划线） |
| 未验证 | 真机：工具栏选区包裹、光标落位、折行开关、渲染观感 |
| 文件所有权 | Claude（cards.tsx / note-md.tsx / workspace.css / i18n） |
| 附带 | 树拖拽裁切修复（e1169ed，拖动行不再被行槽裁出"洞"）；结构待办：视图骨架提升（TopBar/StatusBar App 级单点）已登记待用户确认 |
| 下一工作项候选 | F4 年视图热力 / J2 自动更新 / 渲染层测试基建（等用户点名） |
