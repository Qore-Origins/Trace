...
- 发布引用（2026-09-26）：v0.17.0 发布状态核对时，GitHub/Gitee `main` 为 `14c82294f1cad702ccd4b68d671b35fcefa62b81`；双端 annotated tag object 为 `12941bdcf036aa9ff566fa236a01c8d307ea915b`，peeled commit 为 `403fcb174667743f2979ba55b9d29b5ef35ce4f4`。Gitee Release ID `1168823` 于 `2026-09-26T15:13:45Z` 发布，页面 `https://gitee.com/Qore/trace/releases/v0.17.0`；正文与本地发行说明匹配，未上传 EXE，API 返回的 `v0.17.0.zip`、`v0.17.0.tar.gz` 为平台自动源码归档。GitHub Release：`https://github.com/Qore-Origins/Trace/releases/tag/v0.17.0`，预发布；包 210,577,133 bytes，SHA-256 `394DFFA219B9329B150D19D33DD0887FC67619551AE7E38093DF841E39B41699`，GitHub API digest 与本地一致。精确当前 HEAD/远端 main 以接手时 `git log` 与 `git ls-remote` 实测为准。隔离分支 `codex/trace-task10-11` 保留发布准备提交 `f769415676bc3ebe7676bbc4f6902b8206679a0f`。主目录未跟踪的 `Resource/pic/`、`Resource/vid/` 是用户资源，必须保留且不触碰。用户确认凭据已轮换；API 调用仅从用户级环境变量在短生命周期 PowerShell 进程内读取，未打印或持久化凭据。当前 Codex 进程本身未继承该环境变量。
